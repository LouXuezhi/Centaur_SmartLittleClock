...
//  static char _msg[100] = "hello world";
...
void astra_ui_main_core(void)
{
  ...
  if (!in_astra) return;
  if(Hour_Setting_Flag) 
  {
	  GetTime.Hours=Hours; 
	Reset_RTC_Time(&GetTime);
	  Hour_Setting_Flag=false;
  }
  if(Minute_Setting_Flag) 
  {
	  GetTime.Minutes=Minutes;  
	  Reset_RTC_Time(&GetTime);
	  Minute_Setting_Flag=false;
  }
  if(Second_Setting_Flag) 
  {
	  GetTime.Seconds=Seconds; 
	Reset_RTC_Time(&GetTime);	  
	  Second_Setting_Flag=false;
  }
  //切换in user item的逻辑
