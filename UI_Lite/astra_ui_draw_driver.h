...
#define get_ticks() HAL_GetTick() // 获取系统时间（毫秒）
#define delay(ms) HAL_Delay(ms)   // 毫秒级延时
...
//#define oled_draw_H_dotted_line(x, y, l) u8g2_DrawHLine(&u8g2, x, y, l) // U8G2 不支持虚线，使用普通线代替
//#define oled_draw_V_dotted_line(x, y, h) u8g2_DrawVLine(&u8g2, x, y, h) // 同上
...
//#define oled_send_area_buffer(x, y, w, h) u8g2_SendBuffer(&u8g2) // U8G2 不支持区域发送，使用全屏发送代替
//#define u8g2_font_my_chinese u8g2_font_unifont_t_symbols
...
#define u8g2_font_my_chinese u8g2_font_wqy12_t_chinese1
/* 此处修改oled绘制函数 */
...
extern uint8_t u8g2_gpio_and_delay_stm32(U8X8_UNUSED u8x8_t *u8x8, U8X8_UNUSED uint8_t msg, U8X8_UNUSED uint8_t arg_int, U8X8_UNUSED void *arg_ptr);
