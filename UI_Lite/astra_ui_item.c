...
//void *_font -> uint8_t *_font
void astra_set_font(const uint8_t *_font)
{
...
{
	//if (astra_selector.selected_item-> layer== 1) return;
  if (astra_selector.selected_item->type == slider_item && astra_to_slider_item(astra_selector.selected_item)->is_confirmed)
  {
    astra_slider_item_t* _selected_slider_item = astra_to_slider_item(astra_selector.selected_item);
    *_selected_slider_item->value -= _selected_slider_item->value_step;
    ...
    if(strncmp((_selected_slider_item->base_item).content,"Hours",5)==0) Hour_Setting_Flag=1;
	if(strncmp((_selected_slider_item->base_item).content,"Minutes",7)==0) Minute_Setting_Flag=1;
	if(strncmp((_selected_slider_item->base_item).content,"Seconds",7)==0) Second_Setting_Flag=1;
	  return;
  }
...
void astra_selector_jump_to_selected_item(void)
{
  ...
  if (astra_selector.selected_item->type == slider_item)
  {
    ...
    if (_selected_slider_item->is_confirmed)
    {
      ...
      _selected_slider_item->is_confirmed = false;
		if(strncmp(_selected_slider_item->base_item.content,"Hours",5)==0) Hour_Setting_Flag=true;
		if(strncmp(_selected_slider_item->base_item.content,"Minutes",7)==0) Minute_Setting_Flag=true;
		if(strncmp(_selected_slider_item->base_item.content,"Seconds",7)==0) Second_Setting_Flag=true;
      return;
    }
  }
  ...
  {
	 //HAL_UART_Transmit(&huart1,(const uint8_t*)(astra_selector.selected_item->content), strlen(astra_selector.selected_item->content), 1000);
		
	 // SPI_Send();
	  //astra_push_info_bar("successfully chosen",3000);
	  //astra_push_info_bar("! ", 2000);
	  astra_push_pop_up(" ",1500);
	  if (strncmp(astra_selector.selected_item->content,"24",5)==0)
	  {
		   if24timesys=true;
	  }
	  if (strncmp(astra_selector.selected_item->content,"12",5)==0)
	  {
...
{
	if (astra_selector.selected_item-> layer== 1&&astra_selector.selected_item->type!=user_item) return;
  if (astra_selector.selected_item->type == slider_item && astra_to_slider_item(astra_selector.selected_item)->is_confirmed)
  {
    //如果已选中又长按退出键
    astra_slider_item_t* _selected_slider_item = astra_to_slider_item(astra_selector.selected_item);
...
//	
//	HAL_SPI_Transmit(&hspi1, (const uint8_t*)(astra_selector.selected_item->content), strlen(astra_selector.selected_item->content), 1000);
//	HAL_GPIO_WritePin(GPIOA,4,GPIO_PIN_SET);
//}

//uint8_t identify(char* content)
//{
//	if(strncmp(content,""))
//}
